...
{
// BEGIN: T5
    int_t my_id = (int_t)id;
    //printf("Hello from %ld\n", my_id);
    // Go through each time step
    for ( int_t iteration=0; iteration<=max_iteration; iteration++ )
    {
        if ( my_id == 0 && (iteration % snapshot_freq)==0 )
        {
            domain_save ( iteration / snapshot_freq );
        }

        pthread_barrier_wait(&barrier);
        //printf("iteration A[%ld]: %ld\n", iteration, my_id);

        // Derive step t+1 from steps t and t-1
        boundary_condition(my_id);
        pthread_barrier_wait(&barrier);
        //printf("iteration B[%ld]: %ld\n", iteration, my_id);
        
        time_step(my_id);
        pthread_barrier_wait(&barrier);
        //printf("iteration C[%ld]: %ld\n", iteration, my_id);
...
// BEGIN: T1c
    ...
    if (pthread_barrier_init(&barrier, NULL, n_threads ) != 0) {
        fprintf(stderr, "Error initializing barrier. Exiting ...\n");
        return -1;
    }
// END: T1b
...
// BEGIN: T2
    for (int_t i = 0; i < n_threads; i++) {
        if (pthread_create(&threads[i], NULL, simulate, (void *)i) != 0) {
            fprintf(stderr, "Error creating thread number %ld. Exiting ...\n", i);
            return -1;
        }
    }
    for (int_t i = 0; i < n_threads; i++) {
