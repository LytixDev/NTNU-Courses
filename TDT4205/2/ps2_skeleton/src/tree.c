...
node_t* node_create ( node_type_t type, void *data, size_t n_children, ... )
{
    ...
    */
    ...
    for (size_t i = 0; i < n_children; i++) {
        node_arg = va_arg(args, node_t *);
        node->children[i] = node_arg;
    }
    va_end(args);
...
static void node_finalize ( node_t *discard )
{
    ...
    */
    ...
    if (discard->type == IDENTIFIER_DATA || discard->type == NUMBER_DATA 
        || discard->type == STRING_DATA)
        free(discard->data);
    free(discard->children); // Frees pointers to the children, not the actual children nodes
    free(discard);
}
