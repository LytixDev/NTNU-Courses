...
// Feel free to add your own macros here if you want to
...
relation :
      expression '=' expression { $$ = node_create ( RELATION, strdup("="), 2, $1, $3 ); } /* I don't like the use of strdup. Would prefer to use an enum so 1. don't have to heap allocate. 2. less computationally intensive to check the data. */
    | expression '!' '=' expression { $$ = node_create ( RELATION, strdup("!="), 2, $1, $4 ); }
    | expression '<' expression { $$ = node_create ( RELATION, strdup("<"), 2, $1, $3 ); }
    | expression '>' expression { $$ = node_create ( RELATION, strdup(">"), 2, $1, $3 ); }
    ;

expression :
      expression '+' expression { $$ = node_create ( EXPRESSION, strdup("+"), 2, $1, $3 ); }
    | expression '-' expression { $$ = node_create ( EXPRESSION, strdup("-"), 2, $1, $3 ); }
    | expression '*' expression { $$ = node_create ( EXPRESSION, strdup("*"), 2, $1, $3 ); }
    | expression '/' expression { $$ = node_create ( EXPRESSION, strdup("/"), 2, $1, $3 ); }
    | expression '<' '<' expression { $$ = node_create ( EXPRESSION, strdup("<<"), 2, $1, $3 ); }
    | expression '>' '>' expression { $$ = node_create ( EXPRESSION, strdup(">>"), 2, $1, $3 ); }
    | '-' expression { $$ = node_create ( EXPRESSION, strdup("-"), 1, $2 ); }
    | '(' expression ')' { $$ = $2; }
    | number { $$ = $1; } 
    | identifier { $$ = $1; } 
    | array_indexing { $$ = $1; } 
...
number :
      NUMBER { 
        ...
        int64_t *n_ptr = malloc(sizeof(int64_t));
        *n_ptr = n;
        $$ = node_create ( NUMBER_DATA, n_ptr, 0 );
      }
    ;
