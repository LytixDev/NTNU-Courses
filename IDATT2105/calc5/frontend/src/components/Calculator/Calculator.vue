<template>
  ...
  <div class="flex-container">
    ...
    <div v-if="$store.state.username">
      <button @click="fetchCalc()">Fetch last calcualations</button>
    </div>
...
export default defineComponent({
  ...
  methods: {
    async fetchCalc() {
      const res = await CalculatorService.fetchCalculations(store.state.token)
      if (res.data) {
        let logArray = []
        for (let obj of res.data)
          logArray.push(obj.equation + " = " + obj.result)

        this.logArray = logArray.reverse()
      }
    },
    sourceAppend(c) {
      this.source += c
    ...
    async parseSource() {
      ...
      let result = "";
      await CalculatorService.postCalculatorExpr(expr, store.state.token)
      .then((value) => {
